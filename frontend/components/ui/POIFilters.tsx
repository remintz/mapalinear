...
export function POIFilters({ filters, activeFilters, onFilterToggle }: POIFiltersProps) {
  ...
  return (
    <div className="sticky top-16 z-10 bg-gray-50/95 backdrop-blur-sm border-b border-gray-200 lg:border-b-0 lg:border-r lg:bg-transparent lg:backdrop-blur-none py-3 mb-4 lg:mb-0 lg:py-0 lg:pr-4">
      {/* Desktop: Title */}
      <h3 className="hidden lg:block text-sm font-semibold text-gray-900 mb-3">Categorias</h3>

      {/* Filters container - horizontal on mobile, vertical on desktop */}
      <div className="flex flex-wrap gap-2 lg:flex-col lg:gap-1">
        {/* Select all button - always visible, disabled when all are selected */}
        <button
          onClick={() => filters.forEach(f => !activeFilters.has(f.id) && handleFilterToggle(f.id))}
          disabled={activeFilters.size === filters.length}
          className={`inline-flex items-center justify-center h-8 px-3 rounded-full lg:rounded-lg lg:w-full lg:justify-start text-xs font-medium border transition-colors ${
            activeFilters.size === filters.length
      ...
      <div className="hidden lg:block mt-4 pt-4 border-t border-gray-200">
        <p className="text-xs text-gray-500">
          {totalActivePOIs} pontos mostrados
        </p>
      </div>
    </div>
  );
